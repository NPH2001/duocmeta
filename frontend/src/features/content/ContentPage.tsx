  ...
  return (
    <article className="mx-auto flex max-w-5xl flex-col gap-10 px-6 py-12 md:py-16">
      <nav aria-label="Breadcrumb" className="text-sm text-emerald-700">
        ...
        <span className="text-emerald-950">{page.title}</span>
      </nav>

      <header className="space-y-5 border-b border-emerald-100 pb-10">
        <p className="text-xs font-semibold uppercase tracking-[0.32em] text-emerald-700">
          {t("content.page")}
        </p>
        <h1 className="max-w-4xl text-4xl leading-tight text-emerald-950 md:text-6xl">{page.title}</h1>
        <time className="block text-sm text-emerald-700" dateTime={page.published_at}>
