  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-10 px-6 py-10 md:py-14">
      <section className="grid gap-5 border-b border-emerald-100 pb-8 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">{t("cart.kicker")}</p>
          <h1 className="mt-3 text-4xl leading-tight text-emerald-950 md:text-5xl">{t("cart.title")}</h1>
        ...
        <Link
          href="/products"
          className="inline-flex justify-center rounded-full border border-emerald-300 px-5 py-3 text-sm font-semibold uppercase tracking-[0.16em] text-emerald-800 hover:border-emerald-950 hover:text-emerald-950"
        >
      ...
      {isLoading ? (
        ...
        <section className="grid gap-8 lg:grid-cols-[1fr_360px] lg:items-start">
          ...
          <aside className="rounded-2xl border border-emerald-100 bg-white/90 p-6 shadow-[0_20px_50px_rgba(6,78,59,0.07)]">
            <h2 className="text-xl text-emerald-950">{t("cart.orderSummary")}</h2>
            ...
            <div className="mt-6 flex items-center justify-between gap-4">
              ...
              <span className="text-xl text-emerald-950">{formatMoney(subtotal, currencyCode)}</span>
            </div>
            <Link
              href="/checkout"
...
function CartLineItem({
  ...
  return (
    <article className="grid gap-5 rounded-2xl border border-emerald-100 bg-white/88 p-5 md:grid-cols-[120px_1fr_auto]">
      <div className="flex aspect-square items-end rounded-xl border border-emerald-100 bg-emerald-50 p-3">
        <span className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">{t("cart.item")}</span>
      </div>

      <div className="min-w-0">
        <Link href={`/products/${item.product.slug}`} className="text-xl text-emerald-950 hover:text-emerald-800">
          {item.product.name}
        </Link>
      ...
      <div className="flex flex-col gap-4 md:items-end">
        <label className="text-sm font-medium text-emerald-800">
          ...
          <input
            ...
            className="h-11 w-24 rounded-full border border-emerald-300 bg-white px-4 text-center text-sm text-emerald-950 outline-none focus:border-emerald-950"
          />
        </label>
        <p className="text-lg text-emerald-950">{formatMoney(lineTotal, currencyCode)}</p>
        <button
          type="button"
          disabled={isUpdating}
          onClick={() => onRemove(item)}
          className="text-sm font-semibold text-red-700 hover:text-red-900 disabled:text-emerald-600"
        >
...
function EmptyCart() {
  ...
  return (
    <section className="rounded-2xl border border-emerald-100 bg-white/88 p-8 text-center">
      <h2 className="text-2xl text-emerald-950">{t("cart.emptyTitle")}</h2>
