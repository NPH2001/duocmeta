      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-8 px-6 py-12 md:py-16">
        <section className="rounded-2xl border border-stone-200 bg-white/88 p-8 text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-stone-600">
            The success page needs an order code from the backend place-order response. Return to the storefront or
            check order history once account pages are available.
          </p>
          <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
            <PrimaryLink href="/products">Browse Products</PrimaryLink>
            <SecondaryLink href="/cart">View Cart</SecondaryLink>
          </div>
        </section>
      </div>
    );
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-10 px-6 py-12 md:py-16">
      <section className="grid gap-8 rounded-2xl border border-stone-200 bg-white/88 p-8 md:grid-cols-[1fr_auto] md:items-center">
        <div>
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7 text-stone-600">
            Your order was created by the backend checkout flow. Payment and fulfillment updates will be confirmed by
            backend order and payment services.
          </p>
        </div>
      ...
      <section className="grid gap-6 md:grid-cols-3">
        <StatusCard title="Payment" value="Pending confirmation" />
        <StatusCard title="Fulfillment" value="Awaiting processing" />
        <StatusCard title="Receipt" value="Email confirmation pending" />
      </section>

      <section className="grid gap-6 rounded-2xl border border-stone-200 bg-white/88 p-6 md:grid-cols-[1fr_auto] md:items-center">
        <div>
          <h2 className="text-2xl text-stone-950">Next steps</h2>
          <p className="mt-3 text-sm leading-7 text-stone-600">
            Keep this order code for support. Detailed order history and payment updates will be surfaced by later
            account and payment tickets.
          </p>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row">
          <PrimaryLink href="/products">Continue Shopping</PrimaryLink>
          <SecondaryLink href="/account/orders">Order History</SecondaryLink>
        </div>
      </section>
    </div>
  );
