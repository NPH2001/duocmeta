...
export function OrderSuccessPage({ orderCode }: OrderSuccessPageProps) {
    ...
    return (
      <div className="mx-auto flex max-w-5xl flex-col gap-8 px-6 py-12 md:py-16">
        <section className="rounded-2xl border border-emerald-100 bg-white/88 p-8 text-center">
          ...
          <h1 className="mt-4 text-3xl leading-tight text-emerald-950 md:text-5xl">
            {t("checkout.orderReferenceMissing")}
          </h1>
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-emerald-900/75">
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-10 px-6 py-12 md:py-16">
      <section className="grid gap-8 rounded-2xl border border-emerald-100 bg-white/88 p-8 md:grid-cols-[1fr_auto] md:items-center">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            {t("checkout.orderReceived")}
          </p>
          <h1 className="mt-4 text-4xl leading-tight text-emerald-950 md:text-5xl">
            {t("checkout.thankYou")}
          </h1>
        ...
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-left md:min-w-72">
          ...
          <p className="mt-3 break-all text-2xl text-emerald-950">{orderCode}</p>
        </div>
      </section>
      ...
      <section className="grid gap-6 rounded-2xl border border-emerald-100 bg-white/88 p-6 md:grid-cols-[1fr_auto] md:items-center">
        <div>
          <h2 className="text-2xl text-emerald-950">{t("checkout.nextSteps")}</h2>
          <p className="mt-3 text-sm leading-7 text-emerald-900/75">{t("checkout.nextStepsDescription")}</p>
        </div>
...
function StatusCard({ title, value }: { title: string; value: string }) {
  return (
    <article className="rounded-2xl border border-emerald-100 bg-white/88 p-6">
      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700">{title}</p>
      <p className="mt-4 text-lg text-emerald-950">{value}</p>
    </article>
  );
}
