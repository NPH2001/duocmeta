  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="rounded-2xl border border-emerald-100 bg-white p-6">
        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">Media</p>
        <h1 className="mt-3 text-4xl leading-tight text-emerald-950">Upload media</h1>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_340px]">
        ...
        <aside className="rounded-2xl border border-emerald-100 bg-white p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Latest Upload</p>
          {lastMedia ? (
            <dl className="mt-4 space-y-3 text-sm text-emerald-800">
...
function MediaDetail({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <dd className="mt-1 break-all font-mono text-xs text-emerald-950">{value}</dd>
    </div>
  );
}
